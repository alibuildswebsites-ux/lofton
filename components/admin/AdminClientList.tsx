        ...
        <div className="bg-background rounded-2xl border border-border overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-accent border-b border-border">
                <tr>
                  <th className="px-6 py-4 font-bold text-muted-foreground text-sm">Name</th>
                  <th className="px-6 py-4 font-bold text-muted-foreground text-sm">Email</th>
                  <th className="px-6 py-4 font-bold text-muted-foreground text-sm">Joined</th>
                  <th className="px-6 py-4 font-bold text-muted-foreground text-sm text-center">Saved Props</th>
                  <th className="px-6 py-4 font-bold text-muted-foreground text-sm text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {currentUsers.map(user => (
                  <tr key={user.uid} className="hover:bg-accent">
          ...
          {totalPages > 1 && (
            <div className="px-6 py-4 border-t border-border flex justify-center gap-2">
              ...
              <span className="px-3 py-1 text-muted-foreground">Page {currentPage} of {totalPages}</span>
              <button 
          ...
          <div className="bg-background rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto flex flex-col">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4 bg-accent p-4 rounded-xl">
                <div>
                  <label className="text-xs font-bold text-muted-foreground/60 uppercase">Name</label>
                  <p className="font-bold text-foreground">{selectedUser.name}</p>
                </div>
                <div>
                  <label className="text-xs font-bold text-muted-foreground/60 uppercase">Email</label>
                  <p className="font-bold text-foreground">{selectedUser.email}</p>
                </div>
                <div>
                  <label className="text-xs font-bold text-muted-foreground/60 uppercase">Joined</label>
                  <p className="font-bold text-foreground">{new Date(selectedUser.createdAt).toLocaleDateString()}</p>
                </div>
                <div>
                  <label className="text-xs font-bold text-muted-foreground/60 uppercase">Saved Count</label>
                  <p className="font-bold text-brand">{selectedUser.savedProperties?.length || 0}</p>
                </div>
              </div>

              <div>
                <h4 className="font-bold text-foreground mb-4">Saved Properties</h4>
                {loadingProps ? (
                  <div className="flex justify-center p-8"><Loader2 className="animate-spin text-brand"/></div>
                ) : userProperties.length === 0 ? (
                  <p className="text-muted-foreground text-sm">No saved properties.</p>
                ) : (
            ...
            <div className="p-6 border-t border-border bg-accent flex justify-between items-center">
               <span className="text-xs text-muted-foreground/60">User ID: {selectedUser.uid}</span>
               <button 
                 onClick={() => handleDelete(selectedUser.uid)}
                 className="bg-background border border-red-200 text-red-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-50"
               >
                 Delete Account
               </button>
            </div>
