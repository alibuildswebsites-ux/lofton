...
export const PropertyListings = () => {
  ...
  useGSAP(() => {
    ...
    return () => mm.revert();
  }, { dependencies: [loading, currentProperties, viewMode] });
  ...
  const FilterSidebar = () => (
    <div className="space-y-8">
      ...
      {/* Status Tabs */}
      <div>
        <h4 className="font-bold text-foreground mb-3">Status</h4>
           ...
           {['All', 'For Sale', 'For Rent', 'Sold', 'Rented'].map((s) => (
             <button
               ...
               className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${
                 filters.status === s ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted'
               }`}
             >
               {s}
             </button>
      ...
      {/* Price Range */}
      <div>
        <h4 className="font-bold text-foreground mb-3">Price Range</h4>
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
             ...
             <input 
               ...
               className="w-full pl-7 pr-3 py-2 border border-border rounded-lg text-sm outline-none focus:ring-2 focus:ring-brand"
             />
          </div>
      ...
      {/* Location */}
      <div>
        <h4 className="font-bold text-foreground mb-3">City</h4>
        <div className="relative">
          <select
            value={filters.location}
            onChange={(e) => handleFilterChange('location', e.target.value)}
            className="w-full appearance-none bg-background border border-border rounded-lg px-4 py-3 pr-10 outline-none focus:ring-2 focus:ring-brand focus:border-transparent text-foreground font-medium"
          >
            <option value="">Any City</option>
            {displayLocations.map(loc => <option key={loc} value={loc}>{loc}</option>)}
          </select>
          <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground/60 pointer-events-none" size={18} />
        </div>
      </div>

      {/* Beds */}
      <div>
        <h4 className="font-bold text-foreground mb-3">Bedrooms</h4>
             ...
             <button
               ...
               className={`h-10 w-10 flex items-center justify-center rounded-lg border text-sm font-semibold transition-all ${
                 filters.beds === num 
                   ...
                   : 'bg-background border-border text-muted-foreground hover:border-brand/50'
               }`}
             >
               {num === 'any' ? 'Any' : `${num}+`}
             </button>
      ...
      {/* Baths */}
      <div>
        <h4 className="font-bold text-foreground mb-3">Bathrooms</h4>
        <div className="flex gap-2">
           {[ 'any', 1, 2, 3, 4 ].map((num) => (
             <button
               key={num}
      ...
      {/* Property Type */}
      <div>
        <h4 className="font-bold text-foreground mb-3">Property Type</h4>
      ...
      <button 
        ...
        className="w-full py-3 flex items-center justify-center gap-2 text-sm font-bold text-muted-foreground hover:text-foreground bg-muted hover:bg-muted rounded-lg transition-colors"
      >
  ...
  return (
    <div className="font-sans bg-accent min-h-screen flex flex-col">
      <Navbar /> 
      
      {/* Header Area */}
      <div className="bg-background border-b border-border pt-28 pb-8">
        <div className="max-w-[1400px] mx-auto px-5 md:px-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
             <div>
               <h1 className="text-3xl md:text-4xl font-extrabold text-foreground tracking-tight">Property Listings</h1>
             </div>
      ...
      <main className="flex-grow max-w-[1400px] mx-auto px-5 md:px-10 py-8 w-full">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="hidden lg:block w-72 flex-shrink-0">
             <div className="bg-background rounded-xl border border-border p-6 sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto">
               <FilterSidebar />
             </div>
          </aside>
            ...
            {loading ? (
              ...
              <div className={`grid gap-6 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1'}`} style={{ perspective: "1200px", transformStyle: "preserve-3d" }}>
                {currentProperties.map(property => (
                  <div key={property.id} className="gsap-list-card" style={{ willChange: "transform, opacity" }}>
                    <PropertyCard 
                      property={property}
                      viewMode={viewMode} 
                    />
              ...
              <div className="flex flex-col items-center justify-center py-20 bg-background rounded-2xl border border-border text-center">
                 <div className="w-16 h-16 bg-muted rounded-full flex items-center justify-center mb-4">
                   <Search className="text-muted-foreground/60" size={32} />
                 </div>
                 <h3 className="text-xl font-bold text-foreground mb-2">No properties match your criteria</h3>
                 <p className="text-muted-foreground max-w-sm mb-6">Try removing some filters or expanding your price range.</p>
                 <button 
                   onClick={clearFilters}
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold transition-all ${
                      currentPage === idx + 1 
                        ...
                        : 'bg-background border border-border text-muted-foreground hover:border-brand hover:text-brand'
                    }`}
                  >
                    {idx + 1}
                  </button>
